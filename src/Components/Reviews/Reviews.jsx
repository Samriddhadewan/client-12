  ...
  return (
    <div className="my-10">
      ...
      <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
        {data.map((item) => (
          <SwiperSlide className="text-center" key={data._id}>
            <Rating
              className="mx-auto"
              style={{ maxWidth: 180 }}
              value={item.rating}
              readOnly
            />
            <p className=" w-28 mt-10 mx-auto">
              <svg
            ...
            <h1 className="text-[#444] mt-10 mx-auto w-7/12 text-base font-normal">
              {item?.comment}
            </h1>
            <div className="flex flex-col items-center mt-6">
              <img
                src={item?.participant_photo}
                alt={item?.participant_name}
                className="w-20 h-20 rounded-full object-cover shadow-md"
              />
              <h1 className="text-[#CD9003] text-2xl font-semibold mt-4">
